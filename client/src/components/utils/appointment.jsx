    ...
    function addAppointment(patientID,diagnosis,medicine,exam,obs){
        ...
        if( isNaN(d_id) || isNaN(p_id) || isNaN(med_code) || isNaN(exam_code) )
        {
            ...
            try{
                ...
                props.contract.methods.appointment(d_id,p_id,diagnosis,med_code,exam_code,timestamp,obs).send( {from: props.accounts[0],gas:3000000} )
            }catch(error){
                console.log(error);
            }
        }
    ...
    return(
        <div>
            ...
            <Button 
                ...
                document.getElementById('Exam').value,document.getElementById('Obs').value)}>
                SUBMIT
            </Button>
        </div>
    )
